#Although it was not adopted in the end, at least....I learnt basic python and....reviewed adaboost...
...
def adaBoost(X,y,B): #program does nothing as written
    def train(X, w, y):
        ...
        for j in range(p):
            ...
            ######################
            for k in range(1, len(w_cum)):
                if w_cum[k] in w_cum[0:k]:
                    w_cum[k] = 0
...
co_app = pd.read_csv('allnames_co_occurrence_v2.csv',index_col=0)
...
for i in range(len(co_app.index)-1):
    for j in range(i+1,n):
        k = i * (2 * n - i - 1) / 2 + (j - i) - 1
        X.iloc[k,0] = co_app.iloc[i,j]

polar = pd.read_csv('polarity_v2.csv',index_col=0)
for i in range(len(polar.index) - 1):
    for j in range(i + 1, n):
        k = i * (2 * n - i - 1) / 2 + (j - i) - 1
        X.iloc[k, 1] = polar.iloc[i, j]

subj = pd.read_csv('subjectivity_v2.csv', index_col=0)
for i in range(len(subj.index) - 1):
    for j in range(i + 1, n):
        k = i * (2 * n - i - 1) / 2 + (j - i) - 1
        X.iloc[k, 2] = subj.iloc[i, j]
...
(len(y) - np.nansum(abs(result - y)))/len(y)

names = X.iloc[[i for i in range(len(result)) if result[i] == 1],3]

import itertools
print list(itertools.chain(*names))

nameset = set(list(itertools.chain(*names)))
